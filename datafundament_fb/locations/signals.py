...
def reorder_property_groups(instance, raw, **kwargs):
...
def reorder_location_properties(instance, raw, **kwargs):
    # Don't run when a fixture is loaded (=raw)
    if not raw:
        # Get order of the modified instance
        instance_order = getattr(instance, 'order', None)
...
def reorder_external_service_properties(instance, raw, **kwargs):
    # Don't run when a fixture is loaded (=raw)
    if not raw:
        # Get order of the modified instance
        instance_order = getattr(instance, 'order', None)

        objects = ExternalService.objects.all().order_by('order')
        index = 1
        for object in objects:
            # skip if the current object is the instance and order has been filled
            if object.id == instance.id and instance_order:
            ...
            # update the object with the index and add 1 for the next
            ExternalService.objects.filter(id=object.id).update(order=index)
            index += 1    
