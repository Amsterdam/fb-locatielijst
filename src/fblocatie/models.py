...
class Adres(models.Model):
    ...
    )  # kan opgehaald via /v1/bag/nummeraanduidingen/
...
class Vastgoed(models.Model):
    ...
    vvo = models.DecimalField(
        verbose_name="Verhuurbaar vloeroppervlak (VVO)", max_digits=10, decimal_places=2, blank=True, null=True
    )
    bvo = models.DecimalField(
        verbose_name="Bruto vloeroppervlakte (BVO)", max_digits=10, decimal_places=2, blank=True, null=True
    )
    energielabel = models.CharField(verbose_name="Energielabel", max_length=5, blank=True, null=True)
    monument_gem = models.ForeignKey(
        MonumentStatus,
        verbose_name="Monument status Amsterdam",
        related_name="monument_gem",
        on_delete=models.RESTRICT,
        blank=True,
        null=True,
    )
    monument_brkpb = models.ForeignKey(
        MonumentStatus,
        verbose_name="Monument status landelijk",
        related_name="monument_land",
        on_delete=models.RESTRICT,
        blank=True,
        null=True,
    )
    themagv = models.ForeignKey(
        ThemaPortefeuille, verbose_name="Themaportefeuille", on_delete=models.RESTRICT, blank=True, null=True
    )
    asset_manager = models.ForeignKey(
        Persoon,
        verbose_name="Assetmanager/contract vastgoed",
        related_name="asset_manager",
        blank=True,
        null=True,
        on_delete=models.RESTRICT,
    )
    pl_gv = models.ForeignKey(
        Persoon,
        verbose_name="Projectleider Gemeentelijk Vastgoed",
        related_name="pl_gv",
        blank=True,
        null=True,
        on_delete=models.RESTRICT,
...
class Locatie(TimeStampMixin):
    ...
    pandcode = models.IntegerField(
        verbose_name="FB pandcode", primary_key=True, default=compute_pandcode
    )  # leidend voor externe leveranciers en datakoppelingen
    afkorting = models.CharField(
        verbose_name="Afkorting", max_length=15
    )  # uniek en identificeerbaar, gelijk aan pandcode in https://hoofdstad.sharepoint.com/sites/in-dga-pa
    ...
    bezoekadres = models.ForeignKey(
        Adres, related_name="bezoek_adres", on_delete=models.RESTRICT, blank=True, null=True
    )
    bezoekadres_functie = models.CharField(
        verbose_name="Functie afwijkend adres", max_length=100, blank=True, null=True
    )
    vastgoed = models.ForeignKey(
        Vastgoed, related_name="locatie_vastgoed", on_delete=models.RESTRICT, blank=True, null=True
    )
    ...
    routecode = models.CharField(
        max_length=15, default="FB"
    )  # dit zijn opties in de huidige db?? gek - location_property_id=14???
    ...
    pas_lc = models.ForeignKey(
        Leverancier1s1p, verbose_name="leverancier van 1s1p", blank=True, null=True, on_delete=models.RESTRICT
    )
