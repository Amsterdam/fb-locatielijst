...
from locations.views import LocationListView, get_csv_file_response
...
class TestLocationListView(TestCase):
    ...
    def setUp(self) -> None:
        ...
        PropertyOption.objects.create(location_property=choice_property, option="KeuzeOptie1")
        PropertyOption.objects.create(location_property=choice_property, option="KeuzeOptie2")
        ExternalService.objects.create(name="External service", short_name="external", public=True)
        Location.objects.create(pandcode=24001, name="Stadhuis", is_archived=False)
        Location.objects.create(pandcode=24002, name="Stopera", is_archived=False)
        Location.objects.create(pandcode=24003, name="Ambtswoning", is_archived=True)
        Location.objects.create(pandcode=24004, name="Kantoor", is_archived=False)
    ...
    def test_search(self, search, location_property, is_staff, archive, expected):
        ...
        is_choice_property = LocationProperty.objects.filter(
            short_name=location_property, property_type="CHOICE"
        ).exists()
...
class LocationCreateViewTest(TestCase):
    ...
    def test_get_view_as_staff_user(self):
        ...
        location_data_names = [
            "naam",
            self.number_property.short_name,
            self.multichoice_property.short_name,
        ]
...
class TestLocationImportForm(TestCase):
    ...
    def test_import_csv_post(self):
        ...
        self.assertEqual(
            LocationData.objects.filter(location=location, location_property__short_name="choice").count(), 1
        )
        self.assertEqual(
            LocationData.objects.filter(location=location, location_property__short_name="multi").count(), 2
        )
...
class TestLocationExport(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.choice_option = PropertyOption.objects.create(location_property=self.choice_property, option="Office")
        self.multichoice_property = LocationProperty.objects.create(
            short_name="multi", label="teams", property_type="CHOICE", multiple=True
        )
        self.multichoice_option1 = PropertyOption.objects.create(
        ...
        self.occupied = LocationData.objects.create(
            location=self.location, location_property=self.boolean_property, value="Ja"
        )
        self.build = LocationData.objects.create(
            location=self.location, location_property=self.date_property, value="31-12-2023"
        )
        self.mail = LocationData.objects.create(
            location=self.location, location_property=self.email_property, value="mail@example.org"
        )
        self.floors = LocationData.objects.create(
            location=self.location, location_property=self.number_property, value="10"
        )
        self.note = LocationData.objects.create(
            location=self.location, location_property=self.memo_property, value="Memo"
        )
        self.postcode = LocationData.objects.create(
            location=self.location, location_property=self.postal_code_property, value="1234 AB"
        )
        self.color = LocationData.objects.create(
            location=self.location, location_property=self.string_property, value="Tekst"
        )
        self.url = LocationData.objects.create(
            location=self.location, location_property=self.url_property, value="https://example.org"
        )
        self.type = LocationData.objects.create(
            location=self.location, location_property=self.choice_property, _property_option=self.choice_option
        )
        self.location_data_option1 = LocationData.objects.create(
            location=self.location,
            location_property=self.multichoice_property,
    ...
    def test_get_csv_file_response_as_staff_user(self):
        ...
        self.assertEqual(
            row["multi"], str(self.multichoice_option1.option) + "|" + str(self.multichoice_option2.option)
        )
...
class TestPropertyOptionDeleteView(TestCase):
    ...
    def test_restricted_error(self):
        ...
        expected_message = (
            f"Optie '{property_option.option}' kan niet verwijderd worden, want er zijn nog locatie(s) aan gekoppeld."
        )
