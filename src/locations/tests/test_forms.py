...
from locations.models import ExternalService, Location, LocationProperty, PropertyOption
...
class TestLocationDataForm(TestCase):
    def setUp(self) -> None:
        ...
        self.choice_option_2 = PropertyOption.objects.create(location_property=self.multichoice_property, option="Bus")
        self.external_service = ExternalService.objects.create(name="Externe service", short_name="extservice")
    ...
    def test_location_property_form_fields(self):
        ...
        undefined_property = LocationProperty.objects.create(
            short_name="undefined", label="Undefined property", property_type="onbekend"
        )
        field = self.location_data_form.fields[self.boolean_property.short_name]
        ...
        self.assertIn(
            f"Er bestaat geen formulierveld voor '{undefined_property.property_type}'.", value_error.exception.__str__()
        )
