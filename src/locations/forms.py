...
def set_external_services_fields() -> dict:
    ...
    for service in external_services:
        fields[service.short_name] = forms.CharField(
            label=service.name, required=False, validators=[validators.valid_string], widget=forms.TextInput
        )
...
class LocationImportForm(forms.Form):
    ...
    csv_file = forms.FileField(
        required=True, label="CSV bestand", help_text=mark_safe("Kies het locatie bronbestand dat je wilt uploaden.")
    )
            ...
            if location_property in location_properties:
                ...
                if options_list:
                    ...
                    self.fields[location_property.short_name] = forms.ChoiceField(
                        label="Wat wil je zoeken", choices=options_list
                    )
